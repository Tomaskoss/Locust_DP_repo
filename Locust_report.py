...
def compute_duration(start_str, end_str):
    try:
        ...
        return f"{int(minutes)} min {seconds:.3f} s" if minutes >= 1 else f"{seconds:.3f} s"
    except Exception:
        return "Unknown"
...
def add_network_traffic_charts(network_file, history_file, story, styles):
    ...
    try:
        ...
        rx_total_kb_nonzero = rx_total_kb.iloc[1:]
        tx_total_kb_nonzero = tx_total_kb.iloc[1:]
        ...
        def build_table(title, rows, color):
            table = Table([[title, "Value"]] + rows, colWidths=[130, 110])
        ...
        tx_total_table = build_table("TX Total [kB]", [["Min", f"{tx_total_min:.2f}"], ["Max", f"{tx_total_max:.2f}"], ["Average", f"{tx_total_avg:.2f}"]], "#9B59B6")
        tx_speed_table = build_table("TX [kB/s]",     [["Min", f"{tx_speed_min:.2f}"], ["Max", f"{tx_speed_max:.2f}"], ["Average", f"{tx_speed_avg:.2f}"]], "#8E44AD")
        rx_total_table = build_table("RX Total [kB]", [["Min", f"{rx_total_min:.2f}"], ["Max", f"{rx_total_max:.2f}"], ["Average", f"{rx_total_avg:.2f}"]], "#4A90E2")
        rx_speed_table = build_table("RX [kB/s]",     [["Min", f"{rx_speed_min:.2f}"], ["Max", f"{rx_speed_max:.2f}"], ["Average", f"{rx_speed_avg:.2f}"]], "#357ABD")

        story.append(Paragraph("<b>Network Traffic Statistics</b>", styles["Heading2"]))
        story.append(Spacer(1, 6))
        grid = Table(
            [[tx_total_table, tx_speed_table], [rx_total_table, rx_speed_table]],
            colWidths=[250, 250]
        )
        ...
        story.append(Paragraph(f"Error creating network charts: {e}", styles["Normal"]))
        story.append(Spacer(1, 12))
...
def create_pdf_report(stats_file, history_file, output_file,
    ...
    start_time, end_time, test_type, target_host, target_ip_meta, used_ips = load_test_times(meta_file)
    duration = compute_duration(start_time, end_time)

    # Použi IP z GUI ak je dostupná, inak z metadát
    ...
    if os.path.exists(logo_path):
        ...
        print(f"Logo '{logo_path}' not found, skipping.")

    # Info Table
    info_data = [
        [Paragraph("Test Type",        wrap_style), Paragraph(str(test_type),                               wrap_style)],
        [Paragraph("Target",           wrap_style), Paragraph(f"{target_host} ({resolved_target_ip})",      wrap_style)],
    ...
    story.append(Paragraph("Test Information", styles["Heading1"]))
    ...
    if comment and comment.strip():
        ...
        story.append(Paragraph("Komentár", styles["Heading2"]))
    ...
    story.append(Paragraph("Overview of Testing", styles["Heading1"]))
    ...
    if os.path.exists(topology_output):
        ...
        story.append(Spacer(1, 6))
        story.append(Image(topology_output, width=450, height=360))
        story.append(Spacer(1, 12))
        ...
        print(f"Warning: topology_diagram.png not found, skipping topology section")

    # Pie Chart
    success_count = req_count - fail_count
    sizes         = [success_count, fail_count] if req_count > 0 else [0, 0]
    ...
    story.append(Paragraph("Reachability / Performance Summary", styles["Heading2"]))
    ...
    if os.path.exists(history_file):
        history_df = pd.read_csv(history_file)
        story.append(Paragraph("Time Series Charts", styles["Heading1"]))
    ...
    # Network Traffic
    story.append(PageBreak())
    story.append(Paragraph("Network Traffic Analysis", styles["Heading1"]))
